...
df['Reviews'] = df['Reviews'].replace({
    'Worst': np.random.rand() * 1,  # Random value between 0 and 1
    'Bad': np.random.rand() * 2 + 1,  # Random value between 1 and 3
    'Good': np.random.rand() + 3,  # Random value between 3 and 4
    'Excellent': np.random.rand() + 4  # Random value between 4 and 5
}).round(1)
...
""")
fig, ax = plt.subplots()
sns.countplot(x='Reviews', data=df, ax=ax)
